...
contract DeployOscillonHookScript is Script {
    ...
    function run() external {
        ...
        ChainlinkOracleAdapter usdcAdapter =
            new ChainlinkOracleAdapter(CL_USDC_USD, ARBITRUM_SEQUENCER, C.MAX_ORACLE_AGE);
        ChainlinkOracleAdapter usdtAdapter =
            new ChainlinkOracleAdapter(CL_USDT_USD, ARBITRUM_SEQUENCER, C.MAX_ORACLE_AGE);
        ChainlinkOracleAdapter daiAdapter =
            new ChainlinkOracleAdapter(CL_DAI_USD, ARBITRUM_SEQUENCER, C.MAX_ORACLE_AGE);
        ChainlinkOracleAdapter crvAdapter =
            new ChainlinkOracleAdapter(CL_CRVUSD_USD, ARBITRUM_SEQUENCER, C.MAX_ORACLE_AGE);
        ...
        _registerSortedPool(hook, USDC, USDT, usdcAdapter, usdtAdapter, 6, 6, "USDC/USDT");
        _registerSortedPool(hook, USDC, DAI, usdcAdapter, daiAdapter, 6, 18, "USDC/DAI");
        _registerSortedPool(hook, USDT, DAI, usdtAdapter, daiAdapter, 6, 18, "USDT/DAI");
        _registerSortedPool(hook, USDC, CRVUSD, usdcAdapter, crvAdapter, 6, 18, "USDC/crvUSD");
    ...
    function _registerSortedPool(
        ...
        if (tokenA < tokenB) {
            ...
            hook.registerPool(key, address(adapterA), address(adapterB), decimalsA, decimalsB);
        } else {
            key = PoolKey({
            ...
            hook.registerPool(key, address(adapterB), address(adapterA), decimalsB, decimalsA);
        }

...
library OscillonPriceEngine {
    ...
    function getSellTokenPrice(
        TokenOracleConfig memory cfg,
        uint256 twapPrice1e18
    ) internal view returns (PriceResult memory result) {
        if (cfg.chainlinkAdapter != address(0)) {
            (bool ok, PriceResult memory fromCl) = _tryChainlink(
                cfg.chainlinkAdapter,
                twapPrice1e18
            );
            if (ok) return fromCl;
        }
    ...
    function _tryChainlink(
        address chainlinkAdapter,
        uint256 twapPrice1e18
    ) internal view returns (bool ok, PriceResult memory result) {
        try IOscillonOracle(chainlinkAdapter).getPrice() returns (
            uint256 clPrice,
            uint256
        ) {
            uint256 finalPrice = OscillonDepegMath.pricesDisagree(
                clPrice,
                twapPrice1e18
            )
    ...
    function _pack(
        ...
        (uint256 depegBps, bool pegBelow) = OscillonDepegMath.depegFromPrice(
            price1e18
        );
        result = PriceResult({
            price1e18: price1e18,

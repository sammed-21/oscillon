...
| Value bound on feed reads | `answer <= 0` reverts | [`ChainlinkOracleAdapter._readFeed`](src/oracle/adapters/ChainlinkOracleAdapter.sol#L70) | **This is the entire sanity-bound check.** There is no upper bound, no max-deviation-from-last-answer check, no circuit breaker on an absurd-but-positive value (e.g. a feed misreporting $1000 due to a decimals bug would pass this check and be treated as a 99,900 bps depeg, clamped only by the disagreement guard in §1.1 if TWAP disagrees). Threat model previously listed "sanity bounds" as a defense against "oracle returns absurd value" — **this row shows that defense is materially thinner than the label implied.** |
...
This is a partial set — it proves the fee-bound claims in §2, not the oracle-cascade claims in §1 (those still rely on the deterministic unit tests in `test/OscillonHook.t.sol`) or anything in §3/§4.
